...
# ----------------------------------------------------------------------------
# Train (cached per settings via session_state)
# ----------------------------------------------------------------------------
    ...
    try:
        ...
        if run_comparison:
            with st.spinner("Training baseline models for comparison..."):
                baseline_scores = run_baseline_models(jdf, split, sequence_length)
        else:
            baseline_scores = {}
...
with tab_chart:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=result["DateTime"], y=result["Actual"],
                              mode="lines", name="Actual", line=dict(width=1.4)))
    fig.add_trace(go.Scatter(x=result["DateTime"], y=result["Predicted"],
                              mode="lines", name="GRU Prediction", line=dict(width=1.2, dash="dot")))
    if len(anomalies):
    ...
    fig.update_layout(
        ...
        xaxis_title="DateTime", yaxis_title="Vehicles / Traffic Volume",
    ...
    fig_resid = px.histogram(result, x="Absolute_Error", nbins=40,
                              title="Distribution of absolute prediction error")
    fig_resid.add_vline(x=threshold, line_dash="dash", line_color="red",
...
with tab_compare:
    ...
    if not run_comparison:
        st.info("Enable **Compare GRU against baseline models** in the sidebar, "
                 "then retrain to populate this tab.")
    else:
        rows = {"GRU (sequential behavior model)": {"MAE": mae, "RMSE": rmse, "R2": r2}}
        ...
        c1, c2 = st.columns(2)
        with c1:
            fig_mae = px.bar(comp_df, x="Model", y="RMSE", color="Model",
                              title="RMSE by model (lower is better)", text_auto=".2f")
            fig_mae.update_layout(showlegend=False)
            st.plotly_chart(fig_mae, use_container_width=True)
        with c2:
            fig_r2 = px.bar(comp_df, x="Model", y="R2", color="Model",
